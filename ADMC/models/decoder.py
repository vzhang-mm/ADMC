...
class Transformer_decoder(nn.Module):
    ...
    def forward(self, tgt, memory, memory_mask):
        # 位置编码
        query_embed = self.pos_emb(tgt)
        pos_embed = self.pos_emb(memory)

        x = self.decoder(tgt, memory, memory_key_padding_mask=memory_mask, pos=pos_embed, query_pos=query_embed)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
                     ...
                     pos: Optional[Tensor] = None,
                     query_pos: Optional[Tensor] = None):
        q = k = self.with_pos_embed(tgt, query_pos)#加上位置编码
...
class PositionalEncoding_(nn.Module):
    def __init__(self, d_embed, seq_len=5000, dropout=0.1):
    ...
    def forward(self, x):
        ...
        # print("x",x.size())
        return self.dropout(x_pos)#layer层会再加上位置信息
...
if __name__ == '__main__':
    ...
    # 训练过程
    output = model(src, tgt, memory_key_padding_mask)
